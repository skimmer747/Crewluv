...
- **Crewluv** branch `feature/commute-stage1-crewluv-drive` (off `main`): latest `00b3420` — **leave/arrive-home threshold countdown** (2026-06-24). For commuters the big "Leaves In" block now targets the home→base **drive start** ("Leaves Home In", `car.fill`) and the top narrative card describes the **work flight** (destination + departure) instead of the drive. New `TripStateResolver.nextDriveToWork(legs:baseAirportCode:at:)` (+5 tests) is the single commute detector both views share, so card and block always agree; non-commuters (home == base) unchanged. The arrive-home side needed NO code — `PartnerStatusReceiver` already overrides `homeArrivalTime` via `TripStateResolver.resolveHomeArrival`, which scans `.drive` legs. Spec/plan: `docs/superpowers/specs/2026-06-24-commute-home-threshold-countdown-design.md` + `plans/2026-06-24-commute-home-threshold-countdown.md`. Uncommitted: `Crewluv.xcodeproj/project.pbxproj` has the **user's** version bump (leave untouched). Verified by user in Xcode; **NOTHING pushed**.
